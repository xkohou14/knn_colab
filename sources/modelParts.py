...
class SpacialPath(nn.Module):
    ...
    def __init__(self, side_outputs, backbone_out, target_x, target_y):
        ...
        self.semantic = nn.Sequential(
            nn.Conv2d(get_channels(backbone_out), get_channels(backbone_out) // 4, kernel_size=3, padding=1),
            nn.Conv2d(get_channels(backbone_out) // 4, 1, kernel_size=3, padding=1),
    ...
    def forward(self, ten_x: List[torch.tensor], gap_tensor):
        ...
        for i, x_in in enumerate(ten_x):
            cam_out = self.cams[i].forward(x_in)
        ...
        for i in range(1, len(cams_outputs)):
            final_outputs.append(cams_outputs[i-1] + cams_outputs[i])
...
class CAM(nn.Module):
    ...
    def __init__(self, channels, resolution):
        ...
        self.inner = nn.Sequential(
            ...
            nn.Conv2d(1, channels, kernel_size=1),  # Learn weight to get appropriate attention to each feature
